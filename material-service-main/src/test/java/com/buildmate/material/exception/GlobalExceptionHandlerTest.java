...
class GlobalExceptionHandlerTest {
    ...
    void setUp() {
        ...
        when(request.getRequestURI()).thenReturn("/materials");
    }
    ...
    void validationReturns400WithFieldDetails() {
        ...
        assertEquals("/materials", response.getBody().get("path"));
    ...
    void notFoundReturns404() {
        ResponseEntity<Map<String, Object>> response = handler.handleNotFound(
                new ResourceNotFoundException("Material not found"), request);

        assertEquals(HttpStatus.NOT_FOUND, response.getStatusCode());
        assertEquals(404, response.getBody().get("status"));
        assertEquals("Not Found", response.getBody().get("error"));
        assertEquals("Material not found", response.getBody().get("message"));
    }
    ...
    void conflictReturns409() {
        ResponseEntity<Map<String, Object>> response = handler.handleConflict(
                new DuplicateResourceException("Brand already exists."), request);
